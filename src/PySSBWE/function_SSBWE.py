...
def SSBWE(spec,df,extra_factor,zp_factor,side_cut=True,order=0,noise_type="white"):
    ...
    output_ssbwe_1 = 1.85*np.fft.fft(y_extra_1,round(zp_factor*len(y_extra_1)))/len(y_extra_1) #Method 1
    output_ssbwe_2 = 1.85*np.fft.fft(y_extra_2,round(zp_factor*len(y_extra_2)))/len(y_extra_2) #Method 2
